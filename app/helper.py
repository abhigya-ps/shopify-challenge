...
def prev_url_checker(url_str):
    ...
    url_list = url_str.split('/')  # split url into list spearated by '/'
    # print(url_list)
    ...
    elif 'tags' in url_list:         # check to see if the previous url is from a category/tags page
...
def file_format(filename):       
    if filename.split('.')[-1].lower() not in image_formats:     # check file format with acceptable formats
        return False
    else:
        return True
